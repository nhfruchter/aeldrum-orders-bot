...
async def on_message(message):
    ...
    async def order(order_content):
     ...
     affiliation = requestor.top_role
     content = order_content

     # Add ' a' as hack to avoid empty splits (e.g. for input string '')
     maybe_affiliation = (order_content + ' a').split()[0]
     if (maybe_affiliation.startswith('[') and maybe_affiliation.endswith(']')):
       affiliation = maybe_affiliation.replace("[", "").replace("]", "")
       content = content[len(maybe_affiliation):].strip()
